    ...
    , modelConfig(nullptr)
{
    ...
    initialized = initializeSpeechServices();
}
...
bool VoiceHandler::initializeSpeechServices()
{
    ...
    // 配置讯飞语音大模型
    const char* xunfeiAsrModelName = "讯飞-语音大模型"; 
    ...
    if (reason == SPEECH_RECOGNIZED) {
        if (text) {
            ...
            if (!recognizedSegment.isEmpty()) {
                ...
                emit handler->textRecognized(handler->accumulatedRecognizedText);
            }
        }
...
void VoiceHandler::playText(const QString &text)
{
    ...
    speech_synthesizer_synthesize_text_async(synthesisSession, 
...
QString VoiceHandler::speechToText(const QByteArray &audioData)
{
    ...
    // 等待结果返回
    // 注意：在实际应用中，应该使用信号槽机制异步获取结果
    // 这里简化处理，直接返回累积的文本
    return accumulatedRecognizedText;
}
